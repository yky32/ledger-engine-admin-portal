...
export function AccountingPanel() {
  ...
  /** GETs only — opening the screen must never write. */
  ...
  /** Explicit seed (was auto-run on load): POST /accounting-rules/ensure. */
    ...
    <PageShell
      ...
      actions={
        <ActionBar loading={loading} error={error} ok={ok}>
          ...
          {executions.length === 0 ? (
            <button type="button" className="btn-primary" onClick={() => void seedSequences()}>
              Seed UA sequences
            </button>
          ) : null}
        </ActionBar>
